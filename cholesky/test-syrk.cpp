...
int main( int argc, char **argv ) {
  ...
  char L = 'L', N = 'N';
  double none = -1., one = 1.;
  dsyrk_( &L, &N, &size, &size, &none, XCopy, &size, &one, TCopy, &size );
  //if( getRank() == 0 )
  //  printMatrix( XCopy, size );
  //printf("\n");
  ...
  */
  ...
  if( getRank() == 0 )
    printf("Receiving X\n");
  */
  colTo1ProcTri( T, TDist, size, r, P );
  //printf("\n");
  if( getRank() == 0 ) {
    double maxDiff = 0.;
    if( getRank() == 0 )
      for( int row = 0; row < size; row++ ) {
	for( int col = 0; col <= row; col++ ) {
	  //printf("%7.1f ", getEntry(T,size,row,col,size/(1<<r))-TCopy[row+col*size] );
	  maxDiff = max(maxDiff, fabs((getEntry(T,size,row,col,size/(1<<r))-TCopy[row+col*size])/TCopy[row+col*size]));
	  //printf("%7.1f %7.1f %7.1f\n", maxDiff, getEntry(T,size,row,col,size/(1<<r)), TCopy[row+col*size]);
	}
	//printf("\n");
      }
