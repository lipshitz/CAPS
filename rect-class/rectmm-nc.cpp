...
/*
  ...
  This version only copies the smaller matrix.  The assumed layout is more complex.  At BFS steps, the two matrices that are split are assumed to be already in their final layouts, at DFS steps they are in recursive layout.
 */
...
void rectBFS( double *A, double *B, double *C, int m, int n, int k, int P, int r, int divm, int divn, int divk, char *patt, int *divPatt ) {
  ...
  double *Ccopies;
  double *LC;
  if( (divm == 1 && divk == 1) || (divn == 1 && divk == 1 ) )
    LC = C;
  else {
    LC = (double*) malloc( sizeof(double)*newCsize );
    Ccopies = (double*) malloc( sizeof(double)*Csize*(divk-1) );
  }
  ...
  double *LA;
  if( (divk == 1 && divn == 1) || (divm == 1 && divn == 1 ) )
    LA = A;
  else {
    LA = (double*) malloc( sizeof(double)*newAsize );
    reduceBy( sub, P, Aargs, LA, Asizes );
  }
  double *LB;
  if( (divk == 1 && divm == 1) || (divn == 1 && divm == 1 ) )
    LB = B;
  else {
    LB = (double*) malloc( sizeof(double)*newBsize );
